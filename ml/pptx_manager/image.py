...
class ImageManager:
    ...
    def get_image_json(self, slide_id: int) -> list[dict]:
        return [
            shape.model_dump(exclude={"blob", "shape_manager"})
            for shape in self.image_frame_shapes[slide_id]
        ]
    ...
    def get_all_image_json(self):
        ...
        for slide_id, shapes in self.image_frame_shapes.items():
            image_frame_json[slide_id] = [
                shape.model_dump(exclude={"blob", "shape_manager"}) for shape in shapes
            ]

        return image_frame_json

    def replace_image(
        self, slide_id: int, shape_id: int | None, new_picture: bytes
    ) -> str:
        """
    ...
    def update_image_frame_shape(
        self, slide_id: int, shape_id: int | None, opts: ImageFrameOpts | dict
    ) -> str:
        """
        ...
        updates = {
            ...
            if getattr(opts, attr) is not None
        }
    ...
    def _get_image_frame(
        self, slide_id: int, shape_id: int | None
    ) -> Iterator[ImageFrameShape]:
    ...
    def _get_image_frame_shape(
        self, slide_id: int, shape_id: int
    ) -> ImageFrameShape | None:
